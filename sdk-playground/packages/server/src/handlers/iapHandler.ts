...
async function getSkus(applicationId: string, env: Env): Promise<IGetSKUs[]> {
	const skusFetch = await fetch(
		`${env.VITE_DISCORD_API_BASE}/applications/${applicationId}/skus`,
		{
			method: 'GET',
	...
	return skusJSON.filter((sku) => {
		...
		const released =
			sku.release_date != null ? new Date(sku.release_date) < new Date() : true;
...
async function getEntitlements(path: string[], request: Request, env: Env) {
	...
	try {
		...
		const entitlements = entitlementsJSON
			...
			.filter(
				(ent) => showTest || ent.type !== EntitlementTypes.TEST_MODE_PURCHASE,
			)
			.map((ent) => {
